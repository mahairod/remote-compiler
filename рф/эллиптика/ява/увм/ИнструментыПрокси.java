...
/**
 * Тип <code>ИнструментыПрокси</code> предназначен для 
 * 
 * @author Антон Астафьев
 * @since Ява-8
 * 
 */
public final class ИнструментыПрокси {
	private ИнструментыПрокси(){}

	public static final Class<? extends СопряжениеТелеМаркер> поискГлавногоСопряжения(СопряжениеТелеМаркер прокси){
		Class<? extends СопряжениеТелеМаркер> телеСопряжение = null;
		for (Class<?> cl: прокси.getClass().getInterfaces()){
			if (СопряжениеТелеМаркер.class.isAssignableFrom(cl)){
				if (телеСопряжение==null || телеСопряжение.isAssignableFrom(cl)){
					телеСопряжение = (Class<? extends СопряжениеТелеМаркер>) cl;
				}
			
			}
		}
		return телеСопряжение;
	}
}
