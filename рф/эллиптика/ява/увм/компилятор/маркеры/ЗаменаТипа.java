...
/**
 * Тип <code>ЗаменаТипа</code> предназначен для 
 * 
 * @author Антон Астафьев
 * @since Ява-8
 * 
 */
public interface ЗаменаТипа<Заменяемый> extends Serializable {
	default void запиши(ObjectOutputStream oos) throws IOException{
		oos.writeObject(заменяемый());
	}
	default void запиши(ObjectInputStream ois) throws IOException, ClassNotFoundException {
		Заменяемый заменяемый = (Заменяемый)ois.readObject();
		оригинал(заменяемый);
	}

	Заменяемый заменяемый();
	void оригинал(Заменяемый заменяемый);
}
